...
static THREAD_LOCAL FILE *vstats_file = NULL;
...
static THREAD_LOCAL int run_as_daemon  = 0;
static THREAD_LOCAL int nb_frames_dup = 0;
static THREAD_LOCAL unsigned dup_warning = 1000;
static THREAD_LOCAL int nb_frames_drop = 0;
static THREAD_LOCAL int64_t decode_error_stat[2];
static THREAD_LOCAL unsigned nb_output_dumped = 0;

static THREAD_LOCAL int want_sdp = 1;

static THREAD_LOCAL BenchmarkTimeStamps current_time;
THREAD_LOCAL AVIOContext *progress_avio = NULL;

static THREAD_LOCAL uint8_t *subtitle_out;

THREAD_LOCAL InputStream **input_streams = NULL;
THREAD_LOCAL int        nb_input_streams = 0;
THREAD_LOCAL InputFile   **input_files   = NULL;
THREAD_LOCAL int        nb_input_files   = 0;

THREAD_LOCAL OutputStream **output_streams = NULL;
THREAD_LOCAL int         nb_output_streams = 0;
THREAD_LOCAL OutputFile   **output_files   = NULL;
THREAD_LOCAL int         nb_output_files   = 0;

THREAD_LOCAL FilterGraph **filtergraphs;
THREAD_LOCAL int        nb_filtergraphs;

#if HAVE_TERMIOS_H

/* init terminal so that we can grab keys */
static THREAD_LOCAL struct termios oldtty;
static THREAD_LOCAL int restore_tty;
#endif
...
static THREAD_LOCAL volatile int received_sigterm = 0;
static THREAD_LOCAL volatile int received_nb_signals = 0;
static THREAD_LOCAL atomic_int transcode_init_done = ATOMIC_VAR_INIT(0);
static THREAD_LOCAL volatile int ffmpeg_exited = 0;
static THREAD_LOCAL int main_return_code = 0;
...
const THREAD_LOCAL AVIOInterruptCB int_cb = { decode_interrupt_cb, NULL };
...
static void do_video_stats(OutputStream *ost, int frame_size)
{
    ...
    if (g_log_output_file_pointer != NULL) {
        vstats_file = g_log_output_file_pointer;
    }
...
static void *ffmpeg_main_thread(void *main_args_void_ptr)
{
    int i, ret;
